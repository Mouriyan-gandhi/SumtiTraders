/* =========================================================
   ORNAMENTS, MOTIFS & JEWELLERY PLACEHOLDERS
   ========================================================= */

const Divider = ({ wide = false }) => (
  <div style={{ display:'flex', alignItems:'center', justifyContent:'center', gap: 14, padding: wide ? '8px 0' : '4px 0' }}>
    <span style={{ flex:1, height:1, background:'linear-gradient(90deg, transparent, rgba(138,109,42,.5))' }}></span>
    <svg width="48" height="14" viewBox="0 0 48 14" fill="none" stroke="#8a6d2a" strokeWidth="0.8">
    ...
    <span style={{ flex:1, height:1, background:'linear-gradient(270deg, transparent, rgba(138,109,42,.5))' }}></span>
  </div>
);

const TinyDiamond = ({ size = 6, color = '#8a6d2a' }) => (
  <span style={{ display:'inline-block', width:size, height:size, background:color, transform:'rotate(45deg)' }} />
);

const Ornament = ({ size = 80 }) => (
  <svg width={size} height={size} viewBox="-50 -50 100 100" fill="none" stroke="#8a6d2a" strokeWidth="0.6" opacity="0.85">
...
const CornerOrnament = ({ size = 100 }) => (
  <svg width={size} height={size} viewBox="0 0 100 100" fill="none" stroke="#8a6d2a" strokeWidth="0.6" opacity="0.45">
    <path d="M0 0 L100 0"/>
    <path d="M0 0 L0 100"/>
...
const Seal = ({ size = 240, label = 'Est. 1970' }) => (
  <svg width={size} height={size} viewBox="-100 -100 200 200" fill="none" stroke="#8a6d2a" strokeWidth="0.6">
    <defs>
...
/* ====== Jewellery line illustrations (placeholders) ====== */
...
const JewelBangle = () => (
  <svg viewBox="-100 -100 200 200" fill="none" stroke="#3a322a" strokeWidth="0.6">
    ...
    {Array.from({length: 24}).map((_,i)=>{
      ...
      return <circle key={i} cx={x} cy={y} r={2} fill="#8a6d2a" stroke="none"/>;
    })}
    {Array.from({length: 12}).map((_,i)=>{
      const a = (i/12)*Math.PI*2 + Math.PI/12;
      const x = Math.cos(a)*54, y = Math.sin(a)*54;
      return <g key={i} transform={`translate(${x},${y}) rotate(${a*180/Math.PI})`}><rect x="-2" y="-2" width="4" height="4" transform="rotate(45)" fill="#8a6d2a"/></g>;
    })}
  </svg>
);
...
const JewelAnklet = () => (
  <svg viewBox="-100 -40 200 80" fill="none" stroke="#3a322a" strokeWidth="0.6">
    ...
    {Array.from({length: 11}).map((_,i)=>{
      ...
      return <g key={i}><circle cx={x} cy={y+6} r="1.6" fill="#8a6d2a" stroke="none"/><path d={`M${x} ${y} L${x} ${y+8}`}/></g>;
    })}
...
const JewelMaang = () => (
  <svg viewBox="-60 -10 120 110" fill="none" stroke="#3a322a" strokeWidth="0.6">
    ...
    {Array.from({length: 6}).map((_,i)=>{
      ...
      return <circle key={'l'+i} cx={x} cy={y} r="1.4" fill="#8a6d2a" stroke="none"/>;
    })}
    {Array.from({length: 6}).map((_,i)=>{
      const t = i/5, x = 50 - t*50, y = 0 + t*30;
      return <circle key={'r'+i} cx={x} cy={y} r="1.4" fill="#8a6d2a" stroke="none"/>;
    })}
    {/* tikka pendant */}
...
const Jewel = ({ kind = 'necklace' }) => {
  const C = placeholders[kind] || JewelNecklace;
  return <C />;
};

window.Divider = Divider;
window.TinyDiamond = TinyDiamond;
