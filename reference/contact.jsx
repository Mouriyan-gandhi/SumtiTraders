/* =========================================================
   CONTACT PAGE — 3 Chennai branches
   ========================================================= */
  ...
  return (
    <div>
      ...
      <section style={{
        ...
        <div style={{ position:'absolute', right: isMobile ? '-40%' : '-15%', top: '-20%',
        ...
        <div style={{ position:'relative', zIndex:2 }}>
          <div className="eyebrow"><TinyDiamond/> &nbsp;<span>Visit · Write · Order</span></div>
          <h1 className="display" style={{ fontSize: isMobile ? 50 : 128, lineHeight: 0.9, marginTop: 18 }}>
      ...
      {/* BRANCH CARDS */}
      <section style={{
        padding: isMobile ? '40px 22px 60px' : '50px 60px 100px',
        ...
        <div style={{
          ...
          {BRANCHES.map((b, i) => (
            <div key={b.name} style={{
              ...
              <div style={{ position:'absolute', top: 18, right: 18, opacity: .45 }}>
                <Ornament size={60}/>
              </div>
              ...
              <h3 className="display" style={{ fontSize: 36, marginTop: 14, lineHeight: 1 }}>
                {b.name.split(' ').map((w,wi)=> wi===b.name.split(' ').length-1 ? <em key={wi}>{w}</em> : <span key={wi}>{w} </span>)}
              </h3>
              ...
              <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginTop: 18 }}>
                ...
                <a className="eyebrow" style={{ color:'var(--ink)', cursor:'pointer' }}>Directions →</a>
              </div>
            </div>
          ))}
        </div>
      ...
      {/* MAP IMPRESSION */}
      <section style={{
        padding: isMobile ? '40px 22px' : '70px 60px',
        ...
        <div className="sect-head">
          <div className="left">
            ...
            <h2 className="display" style={{ fontSize: isMobile ? 32 : 52 }}>
      ...
      <section style={{
        ...
        <div style={{ position:'absolute', inset:0, opacity:.16, backgroundImage:'url(patterns/paisley.svg)', backgroundSize:'320px' }}/>
        <div style={{ position:'relative', display:'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1.4fr', gap: isMobile ? 30 : 60 }}>
          <div>
            <div className="eyebrow"><span className="snum">N° 03</span> · <span>Wholesale Enquiry</span></div>
            <h2 className="display" style={{ fontSize: isMobile ? 36 : 64, lineHeight: 0.95, marginTop: 14 }}>
          ...
          <div style={{ background: 'var(--cream-warm)', padding: isMobile ? 24 : 40, border:'1px solid rgba(138,109,42,.2)' }}>
            {!submitted ? (
              <form onSubmit={(e)=>{ e.preventDefault(); setSubmitted(true); }}>
                  ...
                  <div style={{ display:'flex', gap: 8, flexWrap:'wrap' }}>
                    {['First Touch','Swarnika','FT','All houses'].map(h=>(
                      <label key={h} className="chip" style={{ cursor:'pointer' }}>
                        <input type="checkbox" style={{ marginRight: 6 }} defaultChecked={h==='All houses'}/>
                        {h}
                      </label>
                    ))}
                  </div>
                ...
                <div style={{ marginTop: 20 }}>
                  ...
                  <textarea rows={4} placeholder="Tell us about your store…" style={{
